...
tol = {"vmaxtrans" : 0.2 , "vmaxrot" : 1.0 , "transtol" : 0.09 , "rottol" : 5.0} #m/s   

kp = {"x" : 0.42 ,"y" : 0.52 ,"th":0.085}
ki = {"x" : 0.0056 ,"y" : 0.0033 ,"th":0.00004}
...
def update_setpoint_from_topic(msg):
    ...
    for i in range(num_points):
        ...
        rotate_only = {"x": pose["x"], "y": pose["y"], "th": desired_yaw}
        setpoints.append(rotate_only)

        go_to_target = {"x": x, "y": y, "th": th}
        setpoints.append(go_to_target)

    current_setpoint_index = 0
    processing_setpoint = False

    rospy.loginfo("Setpoints diterima dari topic, total (dengan rotate step): %d", len(setpoints))
    print(setpoints)
