...
navigation_data = pd.read_csv(r"White_map_NewMaps.csv")
...
def update_path():
    ...
    if len(obstacles) > 0:
        ...
        for i, point in enumerate(centerline_points):
            if len(obs_tree.query_ball_point(point, r=0.2)) > 0:
...
def is_path_blocked():
    if len(smoothed_path) == 0:
        return False
    for point in smoothed_path[:5]: 
        for obs in obstacles:
            if np.linalg.norm(np.array(point[:2]) - obs) < 0.2: 
                return True
    return False
...
def pose_callback(msg):
    ...
    current_pose = np.array([msg.pose.position.x, msg.pose.position.y, 0.0])
    #print(f"[POSE] Updated pose: {current_pose}")
...
def scan_callback(msg):
    global obstacles, obs, last_replan_time
    if current_pose is None:
        return
    ...
    for r in msg.ranges:
        if msg.range_min < r < min(msg.range_max, 3.0):
    ...
    #     update_path()
    if is_path_blocked() and rospy.Time.now().to_sec() - last_replan_time > 2.0:
        rospy.loginfo("Obstacle detected! Replanning path...")
...
def publish_target_point():
    global obs, current_pose, robot_yaw
    if len(smoothed_path) == 0:
    ...
    yaw_error = np.arctan2(np.sin(target_yaw - robot_yaw), np.cos(target_yaw - robot_yaw))

    if abs(yaw_error) > 0.2:
        target_msg = Float32MultiArray()
        target_msg.data = [current_pose[0], current_pose[1], yaw_error] 
        target_pub.publish(target_msg)
        print(f"[ROTATE] Rotating to yaw {np.degrees(target_yaw):.1f}°, error {np.degrees(yaw_error):.1f}°")
        return
