...
data = read_csv("trajectory_xy5.csv")
...
for idx in range(len(x)):
    ...
    cord = {"x": x[idx], "y": y[idx], "th": 0}
    setpoints.append(cord)


tol = {"vmaxtrans" : 2.0 , "vmaxrot" : 8.0 , "transtol" : 0.2 , "rottol" : 4.0} #m/s

kp = {"x" : 4.07 ,"y" : 4.05 ,"th":1.20}
...
def controller(setpoint__):
    ...
    if (abs(pose["x"] - setpoint__["x"]) < tol["transtol"] and 
        abs(pose["y"] - setpoint__["y"]) < tol["transtol"]):
        return 1
