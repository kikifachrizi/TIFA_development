...
def callback(msg):
    ...
    if current_setpoint_index < len(setpoints):
        if not processing_setpoint and executing_sequence == True :
        ...
        if executing_sequence == False :
            rospy.loginfo("re-plan processing")
            twist_msg.linear.x = 0
            twist_msg.linear.y = 0
            twist_msg.angular.z = 0

    if current_setpoint_index >= len(setpoints) or executing_sequence == False:
