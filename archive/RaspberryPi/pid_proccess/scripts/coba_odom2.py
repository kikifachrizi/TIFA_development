...
# data = read_csv("~/catkin_ws/src/pid_proccess/scripts/telcaf2.csv")
...
#     cord = {"x": x[idx], "y": y[idx], "th": th[idx]}
#     setpoints.append(cord)



setpoints = [
    {"x": 0.0, "y": 0.0, "th": 90.0}  # Home
    # {"x": 0.0, "y": 0.0, "th": 0.0}  # Setpoint 1
    # {"x": 0.0, "y": 0.0, "th": 90.0}   # Setpoint 2
    # {"x": 2.0, "y": -0.34, "th": 0.0},
...
tol = {"vmaxtrans" : 8.0 , "vmaxrot" : 5.0 , "transtol" : 0.2 , "rottol" : 4.0} #m/s

kp = {"x" : 4.07 ,"y" : 4.05 ,"th":0.020}
ki = {"x" : 0.02 ,"y" : 0.01 ,"th":0.00055}
kd = {"x" : 0.0001,"y" : 0.0001 ,"th":0.002}
...
def controller(setpoint__):
    ...
    print("Th: ", abs(pose["th"] - setpoint__["th"]) < tol["rottol"])
    print("X Pose: ", abs(pose["x"]), "X Point : ", abs(setpoint__["x"]))
    print("Y Pose: ", abs(pose["y"]), "Y Point : ", abs(setpoint__["y"]))
    print("Th Pose: ", abs(pose["th"]), "Th Point : ", abs(setpoint__["th"]))
    # if (abs(pose["x"] - setpoint__["x"]) < tol["transtol"] and 
    #     abs(pose["y"] - setpoint__["y"]) < tol["transtol"] and 
    #     abs(pose["th"] - setpoint__["th"]) < tol["rottol"]):
    #     return 1    
    if (abs(pose["th"] - setpoint__["th"]) < tol["rottol"]):
        return 1    
