...
def update_path():
    ...
    if len(obstacles) > 0:
        ...
        for i, point in enumerate(centerline_points):
            if len(obs_tree.query_ball_point(point, r=0.4)) > 0:
    ...
    smoothed_path = []
    prev_point = None
    for i in range(0, len(corrected_path), 1):
        sample = corrected_path[i:i+1]
        if len(sample) == 0:
            continue
...
def scan_callback(msg):
    ...
    for r in msg.ranges:
        if max(msg.range_min, 0.8) < r < min(msg.range_max, 3.0):
