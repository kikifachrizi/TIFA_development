...
def compute_centerline(points, k=20):
...
def is_path_blocked():
    if len(smoothed_path) == 0:
        return False
    for point in smoothed_path[:15]:
        for obs in obstacles:
            if np.linalg.norm(np.array(point[:2]) - obs) < 0.4:
                return True
    return False
...
def scan_callback(msg):
    ...
    local_obstacles = []
    for r in msg.ranges:
        if max(msg.range_min,0.8) < r < min(msg.range_max, 5.0):
