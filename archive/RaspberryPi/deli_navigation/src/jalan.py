...
class RobotNavigator:
    def __init__(self):
        ...
        # self.goals_list = [(1.0, 0.0), (1.0, -1.0)] 
        self.goals_list = [] 
    ...
    def calculate_linear_velocity(self):
        ...
        if self.current_pose and self.goal_position:
            ...
            linear_velocity_x = min(distance_x+1, 3.0) 
            linear_velocity_y = min(distance_y+1, 3.0)
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.current_goal_index < len(self.goals_list):
                x_goal, y_goal = self.goals_list[self.current_goal_index]
                ...
                x_current = self.current_pose.position.x
                y_current = self.current_pose.position.y
                distance_to_goal = math.sqrt((x_goal - x_current)**2 + (y_goal - y_current)**2)

                if distance_to_goal < 0.2:
                    rospy.loginfo(f"Reached goal {self.current_goal_index + 1}: {self.goals_list[self.current_goal_index]}")
